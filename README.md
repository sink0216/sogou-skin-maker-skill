...
- 根据自家宠物、自推角色或原创人设设计一款新皮肤
- 根据参考图保留角色身份和原有画风，重新安排候选窗
- 没有参考图时，只用文字描述从零设计原创主题
...
设计稿看起来正常，不代表装进输入法后也正常。候选窗会横向拉伸，里面还有拼音、候选词、选中态、翻页按钮和点击区。Windows 和 macOS 的配置与打包方式也不同。
